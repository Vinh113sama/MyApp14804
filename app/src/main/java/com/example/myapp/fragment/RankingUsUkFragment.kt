...
import com.example.myapp.databinding.FragmentRankingUsUkBinding
...
class RankingUsUkFragment : Fragment() {
    private var _binding: FragmentRankingUsUkBinding? = null
    ...
    override fun onCreateView(
        ...
        _binding = FragmentRankingUsUkBinding.inflate(inflater, container, false)
        return binding.root
    }
    ...
    private fun setupRecyclerView() {
        adapter = SongAdapter()
        binding.rcRating2Songs.layoutManager = LinearLayoutManager(requireContext())
        binding.rcRating2Songs.adapter = adapter

        binding.rcRating2Songs.addOnScrollListener(object : RecyclerView.OnScrollListener() {
            override fun onScrolled(recyclerView: RecyclerView, dx: Int, dy: Int) {
                ...
                if (!viewModel.isLoading.value && !viewModel.isLastPage.value &&
                    ...
                    viewModel.loadSongs(SongType.USUK)
                }
            }
        })
    }
    ...
    override fun onResume() {
        super.onResume()
        viewModel.refresh(SongType.USUK)
    }
